...
|-----------|-------------|-----------|-------------------|
| Sequential | Internal SRAM | 5288 | 1.00x |
| Random | Internal SRAM | 6092 | 1.15x |
| Sequential | External Memory | 19616 | 3.71x |
| Random | External Memory | 20347 | 3.85x |
...
เพราะ CPU อ่านข้อมูลที่อยู่ติดกันได้รวดเร็ว โดยใช้ cache ช่วย แต่ random access ข้ามตำแหน่ง ทำให้ cache ใช้ไม่เต็มที่

2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
SRAM อยู่ในชิป เร็วกว่า ส่วน external memory อยู่ข้างนอก ช้ากว่าเพราะต้องส่งข้อมูลผ่านสายภายนอก

3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
stride ใหญ่ → อ่านข้อมูลน้อยลง → เร็วขึ้น
แต่ไม่ได้อ่านครบทุกตำแหน่ง → เหมาะกับบางงานเท่านั้น
---
...
| Average Time per Iteration (μs) | 78 | 9620 |
| Total Execution Time (ms) | 4993 | 5944 |
...
| Average Latency (μs) | ~12,000 |
| Queue Overflow Count | 0 |

### คำถามวิเคราะห์

1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
Core 0 (PRO_CPU) เหมาะกับงานที่ เร็วและใช้ CPU น้อย เช่นงานประสานงานหรือ logic เบา
Core 1 (APP_CPU) เหมาะกับงานที่ ประมวลผลหนัก เช่นงานคำนวณแบบ floating point

2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
ค่า latency เฉลี่ย ~12,000 μs (12 ms) ต่อข้อความ → ถือว่าสูงพอสมควร ต้องระวังถ้าต้องส่งข้อความจำนวนมากหรือต้องการตอบสนองเร็ว

3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
ยังไม่สมดุลดีนัก — Core 1 ใช้เวลามากกว่า Core 0 หลายเท่า
→ อาจปรับให้ workload กระจายเท่ากันกว่านี้เพื่อใช้ศักยภาพทั้งสอง core ได้เต็มที่
---
...
**ข้อมูลนักศึกษา:**
- ชื่อ: นายกันตพัฒน์ ตั้งกิตติธารา
- รหัสนักศึกษา: 66030010
- วันที่ทำการทดลอง: 8/19/2025
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
   ใช้ง่ายขึ้น ไม่งงเหมือนสัปดาห์ที่ 4 รู้วิธี build และ run โปรแกรมได้คล่องขึ้น

2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
   รู้ว่า ESP32 มี 2 core แยกกันทำงานได้ และสามารถสื่อสารกันผ่าน queue

3. ความท้าทายที่พบในการทำ architecture analysis:
   วัดเวลาแต่ละ core ยาก ต้องเข้าใจการทำงานของ task และการสื่อสารระหว่าง core
